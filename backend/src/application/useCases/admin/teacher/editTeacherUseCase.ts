...
export class EditTeacherUseCase implements IEditTeacherUseCase {
  ...
  async execute(teacherId: string, teacherData: Partial<ITeacher>): Promise<Teacher> {
    try {
      ...
      if (teacherData.subject && typeof teacherData.subject === 'string') {
        const subjectDoc = await this.subjectRepository.findByName(teacherData.subject);
        if (!subjectDoc) {
      ...
      const updatedTeacher = await this.teacherRepository.update(teacherId, teacherData);
