...
export class PaymentUseCase implements IPaymentUseCase{
    ...
    async exicute(page:number,limit:number):Promise<{duesDto:StudentFeeDue[];totalCount:number}>{
        const {studentFeeDue,totalCount} = await this.studentFeeDueRepository.findByLimit(page,limit);
