...
export class ProcessPaymentUseCase implements IProcessPaymentUseCase {
  ...
  async execute(studentId: string, feeDueId: string): Promise<{ order: { id: string }; paymentId: string }> {
    ...
    const order = await this.paymentGateway.createOrder(feeDue.getAmount() * 100, `receipt_${feeDueId}`);
