...
import { SessionAttendanceDTO, StudentAttendance } from '../../../domain/types/interfaces';
...
export class GetRecentSessionAttendanceUseCase implements IGetRecentSessionAttendanceUseCase {
  constructor(
  ...
  async execute(teacherId: string, limit: number = 5): Promise<SessionAttendanceDTO[]> {
    const sessions = await this.sessionRepository.findSessionByTeacherId(teacherId, limit);

    const result: SessionAttendanceDTO[] = [];
    for (const session of sessions) {
      const durations = await this.sessionDurationRepository.findBySessionId(session.id);

      const filteredDurations = durations.filter((duration) => duration.durationSeconds < 3600);

      if (filteredDurations.length === 0) continue;

      const studentIds = filteredDurations.map((duration) => duration.userId);

      const studentsData = await this.studentRepository.findManyByIds(studentIds);
      ...
      const students: StudentAttendance[] = filteredDurations.map((duration) => ({
