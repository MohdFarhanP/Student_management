  ...
  import { ILiveSession, ScheduleLiveSessionDTO } from '../../../domain/types/interfaces';
  ...
  export class ScheduleLiveSession implements IScheduleLiveSessionUseCase {
    ...
    async execute(dto: ScheduleLiveSessionDTO & { sessionId: string }): Promise<ILiveSession> {
      // Validate input
      ...
      if (scheduledTime < new Date(now.getTime() - bufferTime)) {
        throw new ValidationError('Scheduled time cannot be more than 5 seconds in the past');
      }
      ...
      console.log(`Scheduling live session ${savedSession.id} with delay ${delay}ms`);
      await this.sessionQueue.add('live-session', savedSession, { delay: delay < 0 ? 0 : delay });
