...
export class TeacherProfileRepository implements ITeacherProfileRepository {
  async getProfile(email: string): Promise<TeacherEntity | null> {
    ...
    const classData = rawTeacher.assignedClass as unknown as {
      name: string;
    } | null;
    const subjectData = rawTeacher.subject as unknown as {
      subjectName: string;
    } | null;
