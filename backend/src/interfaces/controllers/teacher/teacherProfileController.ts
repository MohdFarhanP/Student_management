...
export class TeacherProfileController implements ITeacherProfileController {
  ...
  async updateProfile(req: Request, res: Response): Promise<void> {
    try {
      const user = req.user as IUser | undefined;
      const profile = req.body as Partial<Teacher>;
      ...
      if (profile.phoneNo && (typeof profile.phoneNo !== 'number' || profile.phoneNo <= 0)) {
      ...
      const updatedProfile = await this.updateTeacherProfileUseCase.execute(profile);
      if (!updatedProfile) {
