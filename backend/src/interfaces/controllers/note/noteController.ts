...
import { INoteController } from './INoteController';
...
export class NoteController implements INoteController {
  ...
  async downloadNote(req: Request, res: Response): Promise<void> {
    try {
      ...
      const fileKey = note.fileUrl.split(
        `https://${this.storageService.bucketName}.s3.${this.storageService.region}.amazonaws.com/`
      )[1];
      const downloadUrl =
