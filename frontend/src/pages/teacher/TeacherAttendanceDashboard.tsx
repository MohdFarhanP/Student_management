...
import { getStudentsByClass, fetchClasses, getWeeklyAttendance } from '../../api/admin/classApi';
...
const TeacherAttendanceDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUpdatedData = async () => {
      ...
      setError(null);
      try {
        const [studentsData, timetableData, attendanceData] = await Promise.all([
          getStudentsByClass(selectedClassId),
          fetchTimetable(selectedClassId),
          getWeeklyAttendance(selectedClassId),
        ]);
        setStudents(
  ...
  if (teacherLoading || loading) return <Suspense fallback={<div>Loading...</div>}><LoadingSpinner /></Suspense>;
              ...
              <div className="card bg-base-100 dark:bg-gray-800 shadow-xl p-6">
                ...
                <ErrorBoundary>
                  <Suspense fallback={<LoadingSpinner />}>
                    {loading ? <LoadingSpinner /> : <AttendanceOverviewChart data={weeklyData} />}
                  </Suspense>
                </ErrorBoundary>
              </div>
            </div>
