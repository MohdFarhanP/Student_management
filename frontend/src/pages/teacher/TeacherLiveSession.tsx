import { useEffect, useState, useRef, useCallback, Suspense, lazy, memo } from 'react';
...
const TeacherLiveSession: React.FC<TeacherLiveSessionProps> = ({ userRole, userId }) => {
  ...
  useEffect(() => {
    console.log('[TeacherLiveSession] Component mounted at 2025-05-19 01:55 PM IST');
    return () => {
      console.log('[TeacherLiveSession] Component unmounted at', new Date().toISOString());
  ...
  useEffect(() => {
    console.log('[Socket Test] Sending test-event at 2025-05-19 01:55 PM IST');
    socket.emit('test-event', { message: 'Hello from client', timestamp: '2025-05-19 01:55 PM IST' });
  ...
  useEffect(() => {
    const getClasses = async () => {
      try {
        ...
        if (fetchedClasses?.length && fetchedClasses[0]._id) {
          setSelectedClassId(fetchedClasses[0]._id);
        }
      } catch (err) {
        const message = 'Failed to load classes: ' + (err instanceof Error ? err.message : String(err));
        setError(message);
        toast.error(message);
      }
    };
    ...
    if (selectedClassId) {
      const fetchStudentIds = async () => {
        try {
          ...
          const message = 'Failed to load student list: ' + (err instanceof Error ? err.message : String(err));
          setError(message);
          toast.error(message);
          setStudentIds([]);
        }
  ...
  useEffect(() => {
    ...
    const onLiveSessionJoined = async ({ roomId, token, participants }: { roomId: string; token: string; participants?: UserInfo[] }) => {
      ...
      try {
        ...
        const joinPromise = agoraClientRef.current.join(appId, roomId, token, userId);
  ...
  const startSession = useCallback(() => {
    if (!sessionTitle || !selectedClassId) {
      ...
      toast.error('Session title and class are required.');
      return;
    }
    ...
    console.log('[startSession] Emitting schedule-live-session at', new Date().toISOString(), { sessionId, title: sessionTitle });
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      setLoading(false);
      setError('Failed to start session: Server timeout.');
      toast.error('Session start timed out.');
    }, 10000);
  ...
  const scheduleSession = useCallback(() => {
    if (!sessionTitle || !scheduledAt || !selectedClassId) {
      ...
      toast.error('Session title, class, and date/time are required.');
      return;
    }
    if (!studentIds || studentIds.length === 0) {
      setError('No students found in the selected class');
    ...
    setLoading(true);
    const sessionId = Date.now().toString();
    console.log('[scheduleSession] Emitting schedule-live-session at', new Date().toISOString(), { sessionId, title: sessionTitle });
    socket.emit('schedule-live-session', {
      sessionId,
      title: sessionTitle,
      classId: selectedClassId,
    ...
    toast.success(`Session "${sessionTitle}" scheduled for ${new Date(scheduledAt).toLocaleString()}`);
    setSessionTitle('');
    setScheduledAt('');
    setLoading(false);
    if (loadingTimeoutRef.current) clearTimeout(loadingTimeoutRef.current);
  ...
  const endSession = useCallback(async () => {
    if (sessionInfo) {
      console.log('[endSession] Emitting end-live-session at', new Date().toISOString(), sessionInfo.sessionId);
...
export default memo(TeacherLiveSession);